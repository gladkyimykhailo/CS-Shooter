...
function beginFight(){if(modal!=='shop'||!buyAvailable())closeDialog(false);paused=false;phase='fight';clock=MATCH.roundTime;keys.clear();shootHeld=false;touchFire=false;$('#game-message').textContent=player.team?'T · ВСТАНОВИ C4 НА A АБО B':'CT · ЗАХИЩАЙ ТОЧКИ A / B';setTimeout(()=>{if(state==='playing'&&phase==='fight'&&player.hp>0)$('#game-message').textContent='';},1800);sound('win');updateHUD();}
