...
function sound(kind='shot',weapon='pistol'){
  ...
  if(kind==='shot'||kind==='step'||kind==='hit'){
    ...
    const n=audio.createBufferSource();n.buffer=buffer;const filter=audio.createBiquadFilter();filter.type='lowpass';filter.frequency.value=kind==='step'?400:kind==='hit'?1200:weapon==='shotgun'?1800:2800;n.connect(filter);filter.connect(g);g.gain.value=v*(kind==='shot'?.38:kind==='step'?.1:.12);n.start();
...
function actor(x,y,team,name){return {x,y,team,name,hp:100,armor:0,angle:team?Math.PI*1.2:.6,cooldown:1.4+Math.random(),path:[],pathTimer:0,moving:false,flash:0,seen:0,money:2500,primary:null,weapon:'pistol',inventory:{pistol:{ammo:12,reserve:36}},shots:0};}
function start(){
...
function nextRound(){
  ...
  actors.forEach((a,i)=>{const pos=a.team?map.red[i-3]:map.blue[i];a.x=pos[0];a.y=pos[1];a.angle=a.team?3.8:.72;
    ...
    a.hp=100;a.path=[];a.pathTimer=0;a.cooldown=1.5;a.flash=0;a.seen=0;a.shots=0;
...
function shoot(){
  ...
  for(let pellet=0;pellet<w.pellets;pellet++){
    ...
    for(const {a,d,da} of candidates){
      ...
      const head=height>.8,falloff=player.weapon==='shotgun'?clamp(1-d/14,.15,1):1;damage(a,w.damage*falloff*(head?2:1),player,head);anyHit=true;break;
    }
  }
...
function drawGun(w,h){
  const pistol=player.weapon==='pistol',shotgun=player.weapon==='shotgun',moving=player.moving&&settings.motion;
  ...
  poly([[-105,0],[-88,-85],[-34,-141],[20,-112],[52,0]],tint(skin,.8));poly([[-87,-80],[-72,-123],[-20,-155],[19,-118],[-16,-69]],glove);
